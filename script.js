...
function pullWeatherData(data) {
  ...
  // currentUVIndex.addClass('highRisk');
  indexChanger(data);
}

function indexChanger(data) {
    currentUVIndex.removeClass("zeroRisk");
    currentUVIndex.removeClass("lowRisk");
    currentUVIndex.removeClass("mediumRisk");
    currentUVIndex.removeClass("highRisk");
    currentUVIndex.removeClass("veryHighRisk");
    if (data.current.uvi <= 2) {
    currentUVIndex.addClass("zeroRisk");
  } else if (data.current.uvi <= 5) {
    currentUVIndex.addClass("lowRisk");
  } else if (data.current.uvi <= 7) {
    currentUVIndex.addClass("mediumRisk");
  } else if (data.current.uvi <= 10) {
    currentUVIndex.addClass("highRisk");
  } else if (data.current.uvi > 10) {
    currentUVIndex.addClass("veryHighRisk");
  }
}
    ...
    .then(function (data) {
      ...
      for (var i = 1; i < 6; i++) {
        ...
        date.textContent = new Date(
          (data.daily[i].dt + timeOffset) * 1000
        ).toLocaleDateString();
